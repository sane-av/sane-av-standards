...
- [Appendix A: Extended Guidance](#appendix-a-extended-guidance)
...
- **Designers** should review the documentation and drawing requirements in the [Requirements](#4-requirements) section.
- **DSP programmers** should review the configuration file requirements and the [common suffixes table](#44-reference-suffix-requirements).
- **Installers** should review the labeling requirements in the [Documentation Requirements](#45-documentation-requirements).
- **Commissioning agents** should review the [compliance criteria](#5-compliance).
...
**dBu**: Voltage level referenced to 0.775 V RMS (the voltage that dissipates 1 mW into a 600 Ω load). Originally derived from telephone-line transmission practice; now standard for professional analog audio interconnects.
...
3. A space MUST separate the numerical value from the unit (e.g., `94 dB SPL`). The suffix and the `dB` symbol SHOULD be separated by a space (e.g., `dB SPL`), except for widely established single-block forms where the suffix is attached directly (`dBu`, `dBV`, `dBm`, `dBA`, `dBFS`).
...
4. The form **dB.** (with trailing period as part of the unit symbol) MUST NOT appear. A period MAY appear after the unit only when required by sentence punctuation.
...
**[IEC 60027-3]** Letter symbols to be used in electrical technology, Part 3: Logarithmic and related quantities, and their units. International Electrotechnical Commission, 2002.
